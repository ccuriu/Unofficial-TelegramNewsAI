# Telegram_Digest.exe — Unofficial Telegram News Digest
...
Сценарий использует штатный компилятор `%WINDIR%\Microsoft.NET\Framework64\v4.0.30319\csc.exe` и исходник `Telegram_Digest.cs`. В 5.4.13 Testing пользовательская сборка намеренно не использует Telegram-брендинг или отдельный Telegram-подобный значок; Windows применяет стандартный значок EXE. Результат записывается в `Telegram_Digest.exe`.
...
Самопроверка не запускает Unofficial Telegram News Digest и не обращается к Telegram.
