...
class OfflineRegressionTests(unittest.TestCase):
    ...
    def test_digest_comparison_never_replaces_full_period(self):
        ...
        self.assertIn("lexical_candidate — это подсказка, а не доказательство", request)
        self.assertIn("Не представляй context_message как событие текущего периода", request)
