...
DIGEST_REQUEST = (
    ...
    "changes_since_previous_digest.outside_period_changes содержит старые публикации, изменившиеся после предыдущего выпуска; "
    "учитывай их только для развития сюжета или блока «Что изменилось» и не расширяй ими основной временной интервал. Если "
    "changes_since_previous_digest.comparison_available=true, используй его ссылки для определения новых и содержательно изменённых "
    "публикаций. Сообщения из предыдущего выпуска не исключай из основного дайджеста, если они нужны для полной картины периода. "
    "Изменение только просмотров, реакций, пересылок или комментариев новой новостью не считай. related_message_groups используй "
    "для распознавания перепечаток и общего источника, сохраняя содержательные различия. "
    + EDITORIAL_PRINCIPLES +
    SOURCE_RULES +
    "В заголовке укажи дату и фактический локальный интервал материала по date_local; если надёжно определить интервал нельзя, "
    "не придумывай. При насыщенном материале после заголовка сразу переходи к «Главное за период» из нескольких коротких пунктов; "
