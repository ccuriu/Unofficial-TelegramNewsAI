...
EDITORIAL_PRINCIPLES = (
    ...
    "или оценку полезности материала. Не пиши «для вас», «вам особенно важно». Тематика заранее неизвестна: темы и глубину "
    "определяй по фактическому материалу, без фиксированных рубрик и географических приоритетов. Объединяй публикации только об "
    "одном конкретном эпизоде или его непосредственном развитии: общая тема, страна, организация или война не означают одно событие. "
    "Для развивающегося сюжета сначала установи самое позднее состояние по времени; позднее уточнение имеет приоритет, но не стирай "
    "раннюю версию — при изменении пиши «Первоначально сообщалось…, позже выяснилось…». Перед итоговой формулировкой проверь более "
    "поздние сообщения этого события: опровержение, блокировка, уточнение, отмена или смена статуса должны попасть в итог. Не повышай "
    "уверенность относительно источника: «возможно», «по данным источника» и «предположительно» нельзя превращать в установленный факт. "
    "Различай прямое сообщение, независимое подтверждение, официальное заявление, пересказ первоисточника, инсайд, версию и редакционный "
    "вывод. Названный первоисточник не превращает утверждение в факт. Количество публикаций само по себе не делает сюжет важнее и не "
    "повышает достоверность. Перепечатки одного исходного сообщения не считай независимыми подтверждениями. Уверенность при необходимости: "
    "«подтверждено», «вероятно», «пока не подтверждено», «спорно», «мнение/оценка». Расхождения показывай по совпадающим и спорным деталям. "
    "similar_message_refs и близость формулировок — подсказка; это не доказательство одного события или подтверждения. Не используй общие "
    "знания модели для «улучшения» новости. Не добавляй существенные факты, географию, участников, мотивы, последствия или причинность, "
    "которых нет в переданном материале. Если данных для вывода недостаточно, скажи, что это неясно. Контекст и вывод добавляй только из "
    "материала; если факты самодостаточны, не дописывай обязательную аналитику и не ранжируй событие. Внешние источники используй точечно "
    "для первоисточника, документа, цифры или необходимого контекста. Служебные поля используй только для сопоставления; в готовом ответе "
    "процесс не показывай. Не обсуждай файл, JSON, локальную базу, синхронизацию, дедупликацию или алгоритм поиска. Пиши плотно и "
    "профессионально; сохраняй имена, даты, числа и степень уверенности. Пиши на языке запроса пользователя; если язык запроса не указан "
    "или неясен, пиши по-русски. Иноязычные публикации переводи по смыслу, сохраняя имена, числа, цитируемые факты и ссылки. Не разделяй "
    "источники по языку: русско-, украино- и англоязычные сообщения одного события объединяй в один сюжет. Заголовки делай короткими и "
    "не сильнее данных. "
)


SOURCE_RULES = (
    "Каждый самостоятельный фактический сюжет завершай источником, если есть SOURCE_URL, telegram_url или channel_url. После источника "
    "не добавляй фактов. Если под одним заголовком объединены несколько публикаций, источники должны покрывать существенные утверждения; "
    "иначе раздели сюжет. В «Коротко» ставь источник после каждого события; «Главное за период» может не дублировать ссылки. При наличии "
    "SOURCE_URL/telegram_url используй ровно эту строку, иначе channel_url. Печатай URL обычным текстом, не Markdown-ссылкой: "
    "**Источник:** Канал — https://t.me/...; для нескольких — **Источники:** и затем по одной строке Канал — URL. URL не придумывай и "
    "не меняй: запрещены Markdown redirect, Google redirect, utm_source, сокращение, нормализация, изменение query и восстановление по "
    "памяти. При сомнении копируй literal SOURCE_URL из материала. Для простого сюжета обычно достаточно одного содержательного источника; "
    "для составного — 2–3 ключевых, не приписывая одной ссылке факты, которых в ней нет. Одинаковые перепечатки не перечисляй. "
)


DIGEST_REQUEST = (
    "Подготовь редакторский дайджест за выбранный период. У точных повторов inherited_fields восстанавливай из "
    "родительской публикации по inherits_from_message_key. Основной дайджест всегда строй по всему содержательному материалу "
    "news_messages и operational_messages выбранного периода. continuity_context — предыстория связанных сюжетов; "
    "lexical_candidate не доказательство, context_message не выдавай за текущую новость. changes_since_previous_digest — "
    "только дополнительный слой сравнения: он не задаёт временные границы основного дайджеста и не является фильтром отбора. "
    "outside_period_changes — изменения вне периода; используй их только для развития сюжета или «Что изменилось» и не расширяй "
    "ими основной временной интервал. Если changes_since_previous_digest.comparison_available=true, учти новые и изменённые "
    "публикации. Сообщения предыдущего выпуска не исключай из основного дайджеста, если они нужны "
    "для полной картины периода. Изменение только метрик новой новостью не считай. related_message_groups — только подсказка о "
    "возможном общем источнике, а не приказ объединять сообщения в одно событие. "
    + EDITORIAL_PRINCIPLES +
    SOURCE_RULES +
    "В заголовке укажи дату и фактический локальный интервал всего охваченного материала по date_local, а не только сообщений "
    "из блока сравнения; если надёжно определить интервал нельзя, не придумывай. При насыщенном материале сразу переходи к "
    "«Главное за период»; не ставь перед ним второй абзац с тем же резюме. Если важных событий мало, блок не нужен. Затем раскрой "
    "сюжеты по важности или хронологии; глубину каждого сюжета определяй количеством реально новой информации, обычно 1–3 "
    ...
    "тексте. Отсутствие новых сообщений не считай событием. "
)
