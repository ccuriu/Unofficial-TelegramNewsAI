...
async def sync_channel_once(
    ...
    try:
        ...
        #
        # На первом заполнении второй проход полностью дублировал только что
        # прочитанные сообщения. На последующих запусках его также можно
        # безопасно пропустить, если до начала sync в локальной базе не было
        # известных сообщений внутри refresh-окна. Новые сообщения уже получены
        # первым incremental-проходом. Во всех остальных случаях сохраняем
        # отдельный refresh: перед Stable это проще и надёжнее сложной cursor-схемы.
        refresh_limit = int(
            settings.get(
