...
def load_or_create_credentials():
    if CRED_FILE.exists():
        try:
            ...
            print(
                "Сохраненные данные не трогаю. "
                "Проверьте, что программа запущена под той же учетной записью Windows."
            )
            raise
...
def _digest_created_utc_from_file(path):
    """
    Пытается получить время предыдущего дайджеста из JSON.
    Это нужно при переходе со старых версий, когда runs ещё пустая,
    но предыдущий JSON уже существует.
    """
    try:
        payload = json.loads(
        ...
        if created_utc:
            dt = parse_dt(created_utc)
            if dt:
                return iso_utc(dt)

        created_local = meta.get("created_local")
        if created_local:
            dt = parse_dt(created_local)
            if dt:
                return iso_utc(dt)
...
def get_previous_digest_reference(conn):
    """
    Возвращает:
      (previous_digest_utc, source)

    Приоритет:
    1) последняя зарегистрированная запись в news.db;
    2) ДАЙДЖЕСТ_ПОСЛЕДНИЙ.json;
    3) самый свежий архивный JSON.

    Благодаря этому после обновления программы прошлый дайджест
    не "забывается" только потому, что таблица runs ещё пуста.
    """
    row = conn.execute(
        """
        SELECT created_utc
    ...
    for candidate, source in candidates:
        created_utc = _digest_created_utc_from_file(
            candidate
        )
        if created_utc:
...
DIGEST_REQUEST = (
    "У точных повторов inherited_fields берутся из родительской публикации по inherits_from_message_key. "
    "Сделай редакторский дайджест по этой Telegram-выгрузке, "
    "а не анализ 'фейк/не фейк'. Telegram считай основным "
    "информационным потоком. Если changes_since_previous_digest."
    "comparison_available=true, сначала используй его refs как список "
    "реально новых и содержательно изменённых сообщений; полный текст "
    "найди в news_messages/operational_messages по message_key. "
    "Рост просмотров, реакций, forwards или комментариев сам по себе "
    "не считай новой или изменённой новостью. Используй related_message_groups, "
    "чтобы видеть сообщения с общим источником, но сохраняй важные различия "
    "между версиями каналов. Объединяй сообщения об одном событии, учитывай "
    "более поздние обновления и не считай перепечатки независимыми подтверждениями. "
    "Внешний интернет используй точечно: прежде всего для первоисточника, "
    "документа, оригинального заявления, точных цифр или недостающего контекста, "
    "а не как арбитра над Telegram. Если данные расходятся, просто укажи это; "
    "отдельный блок 'фейки/манипуляции' без необходимости не делай. "
    "Формат: 1) 7–10 главных событий; для каждого 2–4 предложения, почему важно "
    "и какие Telegram-каналы сообщили; 2) Харьков; 3) Война; 4) Украина; "
    "5) Мир; 6) Экономика и деньги; 7) Что обсуждают каналы; "
    "8) Что изменилось с предыдущего дайджеста. Пиши компактно и понятно, "
    "без повторов, чтобы обзор читался примерно за 5–7 минут."
)
...
def build_search_chatgpt_instruction(question, effective_days, search_result):
    return (
        "ЭТО ТЕМАТИЧЕСКАЯ ПОИСКОВАЯ ВЫГРУЗКА ИЗ ЛОКАЛЬНОЙ TELEGRAM-БАЗЫ, "
        "А НЕ ОБЫЧНЫЙ СУТОЧНЫЙ ДАЙДЖЕСТ. Пользователь задал вопрос: "
        f"«{question}». Период поиска: последние {effective_days} дней. "
        "Если после загрузки файла пользователь пишет только «дайджест», "
        "ответь ИМЕННО на этот поисковый вопрос. Сначала дай короткий вывод "
        "«что с этой темой в целом», затем понятную хронологию развития, ключевые "
        "повороты/изменения, кто и когда сообщал, повторяющиеся версии или нарративы, "
        "и что важно на конец периода. search_results — прямые лексические совпадения; "
        "related_context — не прямые совпадения, а посты с тем же исходным origin_key, "
        "их используй только как контекст. related_message_groups помогают видеть "
        "перепечатки/общие источники. Не превращай ответ в автоматический фактчекинг; "
        "внешний интернет используй только если он реально нужен для первоисточника, "
        "документа, точной цифры или контекста. Обязательно посмотри meta.history_completeness: "
        "если complete=true, считай, что локальная история всех выбранных каналов за этот период "
        "была предварительно синхронизирована с Telegram; если complete=false, явно предупреди, "
        "что часть каналов могла быть неполной. Не придумывай отсутствующие в файле события. "
        f"Метод локального поиска: {search_result['method']}; смысловые термины: "
        f"{', '.join(search_result['terms'])}. Для многословного запроса действует минимальное "
        f"покрытие {search_result.get('minimum_term_matches', len(search_result['terms']))}/"
        f"{len(search_result['terms'])}: совпадения только по одному случайному слову отбрасываются. "
        "search_match.phrase_match отмечает точную фразу/языковой вариант; search_match.kind=semantic "
        "означает смысловую подсказку, а не буквальное совпадение."
    )
...
async def _v4_main():
    ...
    try:
        ...
        if previous_run_utc:
            ...
            print(
                "\nЭто первый базовый дайджест. "
                "Следующий запуск уже даст точное сравнение."
            )
...
</style></head><body><header><div class="eyebrow">TELEGRAMNEWSAI · 5.4</div><h1 id="title">Лента новостей</h1><p id="subtitle">Публикации, источники и история изменений</p></header>
