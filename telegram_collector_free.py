...
EDITORIAL_PRINCIPLES = (
    "Охвати весь текущий материал и не ограничивайся несколькими самыми заметными историями: каждый самостоятельный содержательно "
    "значимый сюжет раскрой либо помести в «Коротко»; важный singleton не пропускай. Игнорируй сведения о пользователе, персональную "
    "память и историю чатов: они не источник фактов и не должны влиять на отбор, порядок или акценты. Не пиши «для вас», "
    "«вам особенно важно». Темы и рубрики выбирай по фактическому материалу, без фиксированных рубрик и заранее заданных географических "
    "приоритетов. Объединяй только один конкретный эпизод или его непосредственное развитие; общая тема, страна или организация сами "
    "по себе не означают одно событие. Для развивающегося сюжета сначала установи самое позднее состояние: позднее уточнение имеет "
    "приоритет, но сохраняй развитие; при изменении пиши «Первоначально сообщалось…, позже выяснилось…». Если позднее сообщение "
    "опровергает, уточняет, блокирует, отменяет или меняет статус раннего, отрази это. Не повышай уверенность источника: «возможно», "
    "«по данным источника» и «предположительно» нельзя превращать в факт. Различай прямое сообщение, независимое подтверждение, "
    "официальное заявление, пересказ первоисточника, инсайд, версию и редакционный вывод. Названный первоисточник сам по себе не делает "
    "утверждение подтверждённым. Количество публикаций не определяет важность или достоверность; перепечатки одного исходного сообщения "
    "не считай независимыми подтверждениями. Уверенность при необходимости: «подтверждено», «вероятно», «пока не подтверждено», "
    "«спорно», «мнение/оценка». При расхождениях покажи совпадающие и спорные детали. Связи, похожие формулировки и служебные "
    "relation-подсказки не являются доказательством одного события или подтверждением факта. Не используй общие знания модели для "
    "добавления фактов, географии, участников, мотивов, последствий или причинности. Если данных недостаточно, скажи об этом. Внешние "
    "источники используй только точечно для первоисточника, документа, цифры или необходимого контекста. Если факты самодостаточны, "
    "не дописывай обязательную аналитику и не ранжируй события. Не показывай служебный процесс и не обсуждай файл, JSON, базу, "
    "синхронизацию, дедупликацию или алгоритм. Пиши плотно и профессионально; сохраняй имена, даты, числа и степень уверенности. "
    "Пиши на языке запроса пользователя; если язык запроса не указан или неясен, пиши по-русски. Иноязычные публикации переводи по "
    "смыслу, сохраняя имена, числа, цитируемые факты и ссылки. Не разделяй источники по языку: сообщения одного события объединяй "
    "независимо от языка. Заголовки делай короткими и не сильнее данных. "
)


SOURCE_RULES = (
    "Каждый самостоятельный фактический сюжет завершай строкой источника, если ссылка доступна. В «Коротко» ставь источник после "
    "каждого события; «Главное за период» может не дублировать ссылки. При SOURCE_URL/telegram_url используй исходный URL, иначе "
    "channel_url. Формат: **Источник:** [Канал](URL). URL в скобках копируй ДОСЛОВНО: не придумывай, не подменяй "
    "Google/search/redirect, не добавляй utm_source, не сокращай, не нормализуй, не меняй query и не «исправляй» по памяти. Для простого "
    "сюжета достаточно одного содержательного источника; для составного — 2–3 ключевых, покрывающих существенные утверждения; иначе "
    "раздели сюжет. После строки источника не добавляй новых фактов. Одинаковые перепечатки не перечисляй. "
)


OUTPUT_FORMAT_RULES = (
    "Готовый дайджест выводи обычным сообщением чата, не в writing block, кодовом блоке или другом контейнере, где Markdown-ссылки "
    "могут стать обычным текстом. Любое тематическое уточнение, выбор части материала или изменение объёма ответа меняет только отбор "
    "и подачу: оно не отменяет правила источников. Если для выбранного события доступен SOURCE_URL/telegram_url, обязательно дай "
    "прямую кликабельную ссылку на исходную публикацию Telegram; не заменяй её ссылкой или цитатой на загруженный файл. "
)


DIGEST_REQUEST = (
    OUTPUT_FORMAT_RULES
    + "Подготовь редакторский дайджест за выбранный период. Основной выпуск строй по текущему материалу периода. Предысторию вне периода "
    "используй только как контекст развития, а изменения вне периода — только для развития сюжета или блока «Что изменилось»; ими "
    "нельзя расширять основной временной интервал. В заголовке укажи дату и фактический локальный интервал из переданного материала; "
    "если его надёжно определить нельзя, не придумывай. При насыщенном материале сразу переходи к «Главное за период», без второго "
    "вступительного резюме; если важных событий мало, этот блок не обязателен. Глубину сюжета определяй количеством реально новой "
    "информации, не ограничивая число сюжетов. Значимые события без отдельного разбора собери в «Коротко». Однотипные оперативные "
    "предупреждения одного сюжета объединяй. После «Главное за период» не добавляй повторный итог, личный выбор или рейтинг. "
    "«Что изменилось» добавляй только при существенных изменениях и не используй вместо основного дайджеста. Не объясняй читателю "
    "внутренние правила охвата или сравнения. "
    + EDITORIAL_PRINCIPLES
    + SOURCE_RULES
)
...
CANDIDATE_GUIDANCE = (
    "Каждый CANDIDATE — подсказка группировки, а не утверждение об одном событии. "
    "Объединяй или разделяй кандидаты по EVIDENCE. RELATION — только подсказка, не подтверждение. "
    "MEMBER_REFS и SUPPORTING_REFS обеспечивают покрытие; факты и степень уверенности бери из EVIDENCE. "
    "Значимый singleton не теряй."
)
