...
EDITORIAL_PRINCIPLES = (
    ...
    "Одиночный инсайд не отбрасывай только потому, что его никто не повторил: если он содержательно важен, "
    "включи его с ясной атрибуцией — например, «канал сообщает со ссылкой на источник», «по данным канала» "
    "или «канал утверждает». Отсутствие независимого подтверждения указывай только тогда, когда это существенно "
    "для понимания уверенности в сообщении, без снисходительных формулировок и без автоматического обесценивания инсайда. "
    "Различай по смыслу: наблюдаемое событие или документ; официальное заявление; сообщение со ссылкой на названный "
    "источник; инсайд или ссылка на неназванный источник; аналитическая версия; предположение. "
    "Если разные каналы описывают одно событие по-разному, спокойно сопоставь версии и точно укажи, в чём расхождение. "
    "Не выбирай победителя между версиями без достаточных оснований. Количество перепечаток не является независимым "
    "подтверждением, а позднее содержательное уточнение обычно важнее ранней краткой публикации. "
    ...
    "Пиши естественным профессиональным новостным языком. В готовом ответе никогда не упоминай файл, JSON, выгрузку, "
    "локальную базу, поля, идентификаторы, refs, message_key, синхронизацию, дедупликацию, алгоритм поиска или иные "
    "технические детали подготовки материала. Пользователь должен видеть готовый обзор событий, а не описание обработки данных. "
    ...
    "Сохраняй имена, даты, числа и смысл исходных сообщений; не додумывай отсутствующие детали. "
    "Пиши плотно, ясно и без канцелярита: лучше короче и содержательнее, чем длиннее ради объёма. "
)
...
DIGEST_REQUEST = (
    ...
    "Рост просмотров, реакций, пересылок или комментариев сам по себе не считай новой новостью. "
    "Используй related_message_groups, чтобы распознавать перепечатки и общий источник, но сохраняй важные различия "
    ...
    "точек нет, не придумывай их. Не повторяй дословно одно и то же в кратком блоке и основном тексте. "
)
...
def build_search_chatgpt_instruction(question, effective_days, search_result):
    return (
        ...
        "проверяй их фактическую связь с вопросом и не включай слабые совпадения только ради количества. "
        ...
        "«Что важно сейчас» или «Что важно дальше». "
    )
