...
> [!NOTE]
> **Текущая версия — 5.4.17 Testing.** Уточнены алиасы бренда «Новая почта»/«Нова пошта»: сохранены нужные единственные падежные формы и `Nova Poshta`, а широкие множественные формы больше не создают ложные совпадения. Поисковый JSON теперь также явно содержит версию digest profile. Сетевой профиль 5.4.13 не менялся, его реальные испытания Telegram API продолжаются параллельно. Канал Testing означает, что сетевой профиль ещё проверяется в длительной работе, а не что основная функция проекта приостановлена.
...
![Status](https://img.shields.io/badge/status-5.4.17%20Testing-orange)
