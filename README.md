...
Updater использует единый whitelist `release_manifest.psd1`, не копирует посторонние файлы из временной папки и не удаляет неизвестные файлы в существующей установке. Перед заменой он проверяет, что программа закрыта, сохраняет временную резервную копию заменяемых программных файлов, обновляет зависимости через существующее `.venv`, когда оно совместимо, выполняет проверки и при сбое возвращает прежние программные файлы.
