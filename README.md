...
> [!NOTE]
> **Текущая версия — 5.5.0 Testing.** Перед финальной реальной приёмкой первого Stable точечно усилен MTProto-профиль, а после RUN 1 добавлен плоский `ДАЙДЖЕСТ_ДЛЯ_ИИ.md` для внешних моделей и редакционный профиль 8.3 с приоритетом полноты. Канонический JSON schema 8 остаётся полным техническим экспортом. Telethon остаётся production-транспортом; Web-preview остаётся только исследованной альтернативой. Канал Testing означает, что первый Stable ещё проходит реальную приёмку.
...
После создания обычного дайджеста программа сохраняет два файла рядом: полный канонический `ДАЙДЖЕСТ_ПОСЛЕДНИЙ.json` и пользовательский `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`. В Markdown нет предварительной AI-суммаризации: из него убрана в основном техническая структура, а тексты текущих сообщений, их локальные ссылки и полезный контекст сохраняются. **Проводник Windows автоматически выделяет `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`**, если он создан успешно. Для тематического поиска по-прежнему выделяется `ПОИСК_ПОСЛЕДНИЙ.json`. Постоянные файлы заменяются актуальными версиями при следующем запуске; датированные технические JSON-копии обычных дайджестов сохраняются в `Дайджесты\Архив`, поиска — в `Дайджесты\Архив\Поиск`.
