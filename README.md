...
В JSON-схеме 8 одинаковые `text` и `raw_text` больше не дублируются: если `raw_text_available=true`, но поля `raw_text` нет, исходный текст полностью совпадает с `text`. Если очистка действительно изменила текст, `raw_text` сохраняется целиком. Это уменьшает объём AI-ориентированного файла без потери содержания.
