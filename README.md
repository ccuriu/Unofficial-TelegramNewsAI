...
Если публикация находится в публичном Telegram-канале и для неё доступна ссылка, TelegramNewsAI рекомендует ChatGPT оформлять название канала как кликабельную ссылку на конкретный исходный пост. Внутренние идентификаторы JSON (`refs`, `message_key` и другие служебные поля) используются только для анализа и сопоставления публикаций и не предназначены для отображения пользователю.
