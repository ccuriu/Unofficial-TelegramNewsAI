...
> [!NOTE]
> **Текущая версия — 5.4.15 Testing.** Добавлен штатный безопасный updater для перехода между версиями без замены Telegram-сессии, учётных данных, каналов, настроек, базы и истории. Сетевой профиль 5.4.13 не менялся, его реальные испытания Telegram API продолжаются параллельно. Канал Testing означает, что сетевой профиль ещё проверяется в длительной работе, а не что основная функция проекта приостановлена.
...
![Status](https://img.shields.io/badge/status-5.4.15%20Testing-orange)
