...
> [!NOTE]
> **Текущая версия — 5.5.0 Stable.** Это первый принятый Stable для личного сценария TelegramNewsAI с максимум **50 выбранными источниками**. Канонический JSON остаётся на **schema 8**, AI-facing экспорт — на **digest profile 9.1**. Профиль 9.1 сохраняет candidate-first логику, кликабельные Markdown-ссылки с дословным SOURCE_URL/telegram_url/channel_url и обычный chat-output; тематическое уточнение пользователя меняет только отбор и подачу, не отменяя прямые ссылки на исходные публикации. Редакторский prompt остаётся очищенным от устаревших JSON-имён и повторов и сохраняет правила полноты, достоверности и структуры. **Telethon остаётся production-транспортом**; Web-preview остаётся только исследованной альтернативой.
...
После создания обычного дайджеста программа сохраняет два файла рядом: полный канонический `ДАЙДЖЕСТ_ПОСЛЕДНИЙ.json` и пользовательский `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`. Профиль 9.1 делает Markdown candidate-first: каждое содержательное текущее сообщение попадает ровно в один `CANDIDATE`, сильные связи и консервативный lexical anchor помогают держать развитие одного эпизода рядом, а доказанные near-duplicates могут быть представлены `SUPPORTING_REFS` вместо повторного полного текста. Локальный слой не пишет заголовок события, summary, важность или truth-оценку; полный канонический JSON schema 8 не меняется. **Проводник Windows автоматически выделяет `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`**, если он создан успешно. Для тематического поиска по-прежнему выделяется `ПОИСК_ПОСЛЕДНИЙ.json`. Постоянные файлы заменяются актуальными версиями при следующем запуске; датированные технические JSON-копии обычных дайджестов сохраняются в `Дайджесты\Архив`, поиска — в `Дайджесты\Архив\Поиск`.
...
Для обычного пользователя предназначен отдельный переносимый файл **`TelegramNewsAI_Update.cmd`**. Он не привязан к папке программы: файл можно хранить на рабочем столе, в «Загрузках» или рядом с новым release ZIP.

1. Скачайте новый release ZIP и `TelegramNewsAI_Update.cmd`.
2. Положите ZIP рядом с updater или просто запустите updater и выберите ZIP через обычное окно выбора файла.
3. Updater сам ищет установленный Unofficial TelegramNewsAI: сначала по сохранённому пути установки в профиле Windows, затем по ярлыкам. Если установка не найдена однозначно, он предлагает выбрать папку.
4. Если программа сейчас работает, updater **не закрывает её принудительно**. Текущий запуск можно закончить в обычном порядке; updater дождётся закрытия и продолжит автоматически.
5. Новый release распаковывается только во временную папку, после чего запускается штатный проверенный `update.ps1`.

Путь установки сохраняется в `HKCU\Software\Unofficial TelegramNewsAI\InstallPath` после установки и успешного обновления. Это не секрет и не содержит Telegram-данных. Старый способ тоже остаётся доступным: распакуйте release ZIP и запустите находящийся внутри `UPDATE.bat`; при необходимости папку можно передать явно:

```bat
UPDATE.bat "D:\путь\к\Unofficial TelegramNewsAI"
```

Оба способа используют единый whitelist `release_manifest.json`, не копируют посторонние файлы и не удаляют неизвестные файлы в рабочей установке. Перед заменой программных файлов создаётся временная резервная копия; зависимости обновляются через существующее `.venv`, когда оно совместимо; после обновления выполняются проверки, а при сбое программные файлы откатываются.
