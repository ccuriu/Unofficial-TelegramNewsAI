...
> [!NOTE]
> **Текущая версия — 5.5.0 Testing.** Перед финальной реальной приёмкой первого Stable точечно усилен MTProto-профиль: межканальная пауза по умолчанию увеличена до 1,0 с, лишний recent-refresh убран там, где он ничего не добавляет, а каждый обычный sync пишет локальный `API_SAFETY_SUMMARY`. Telethon остаётся production-транспортом; Web-preview остаётся только исследованной альтернативой. Канал Testing означает, что сетевой профиль ещё проверяется в реальной работе, а не что основная функция проекта приостановлена.
