...
> [!NOTE]
> **Текущая версия — 5.5.0 Stable.** Это первый принятый Stable для личного сценария TelegramNewsAI с максимум **50 выбранными источниками**. Канонический JSON остаётся на **schema 8**, AI-facing экспорт — на **digest profile 8.9**. Профиль 8.9 сохраняет candidate-first логику 8.8, кликабельные Markdown-ссылки с дословным SOURCE_URL/telegram_url/channel_url и дополнительно требует выводить готовый дайджест обычным сообщением чата, а не в writing/code block. **Telethon остаётся production-транспортом**; Web-preview остаётся только исследованной альтернативой.
...
После создания обычного дайджеста программа сохраняет два файла рядом: полный канонический `ДАЙДЖЕСТ_ПОСЛЕДНИЙ.json` и пользовательский `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`. Профиль 8.9 делает Markdown candidate-first: каждое содержательное текущее сообщение попадает ровно в один `CANDIDATE`, сильные связи и консервативный lexical anchor помогают держать развитие одного эпизода рядом, а доказанные near-duplicates могут быть представлены `SUPPORTING_REFS` вместо повторного полного текста. Локальный слой не пишет заголовок события, summary, важность или truth-оценку; полный канонический JSON schema 8 не меняется. **Проводник Windows автоматически выделяет `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`**, если он создан успешно. Для тематического поиска по-прежнему выделяется `ПОИСК_ПОСЛЕДНИЙ.json`. Постоянные файлы заменяются актуальными версиями при следующем запуске; датированные технические JSON-копии обычных дайджестов сохраняются в `Дайджесты\Архив`, поиска — в `Дайджесты\Архив\Поиск`.
