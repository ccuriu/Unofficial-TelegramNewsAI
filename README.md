...
После создания обычного дайджеста программа сохраняет два файла рядом: полный канонический `ДАЙДЖЕСТ_ПОСЛЕДНИЙ.json` и пользовательский `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`. Профиль 8.6 делает Markdown candidate-first: каждое содержательное текущее сообщение попадает ровно в один `CANDIDATE`, сильные связи и консервативный lexical anchor помогают держать развитие одного эпизода рядом, а доказанные near-duplicates могут быть представлены `SUPPORTING_REFS` вместо повторного полного текста. Локальный слой не пишет заголовок события, summary, важность или truth-оценку; полный канонический JSON schema 8 не меняется. **Проводник Windows автоматически выделяет `ДАЙДЖЕСТ_ДЛЯ_ИИ.md`**, если он создан успешно. Для тематического поиска по-прежнему выделяется `ПОИСК_ПОСЛЕДНИЙ.json`. Постоянные файлы заменяются актуальными версиями при следующем запуске; датированные технические JSON-копии обычных дайджестов сохраняются в `Дайджесты\Архив`, поиска — в `Дайджесты\Архив\Поиск`.
