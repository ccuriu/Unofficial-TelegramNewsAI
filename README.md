...
> [!NOTE]
> **Текущая версия — 5.5.0 Testing.** Добавлен ограниченный `continuity_context`: дневной JSON может приложить небольшую предысторию из последних сопоставимых дайджестов, когда сегодняшние публикации продолжают недавний сюжет. Основной период и статистика текущего выпуска не расширяются; старый контекст используется только как помощь для понимания развития событий. Сетевой профиль 5.4.13 не менялся, его реальные испытания Telegram API продолжаются параллельно. Канал Testing означает, что сетевой профиль ещё проверяется в длительной работе, а не что основная функция проекта приостановлена.
...
- использует умеренный последовательный сетевой профиль: 0,5 с между последовательными запросами большой истории и короткую паузу 0,25 с между каналами в поддерживаемом диапазоне до 50 источников;
