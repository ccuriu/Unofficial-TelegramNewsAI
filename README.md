...
> [!NOTE]
> **Текущая версия — 5.4.16 Testing.** Исправлена ложная группировка разных новостей по постоянным профильным и канальным ссылкам: такие URL больше не считаются доказательством общего конкретного материала. Сетевой профиль 5.4.13 не менялся, его реальные испытания Telegram API продолжаются параллельно. Канал Testing означает, что сетевой профиль ещё проверяется в длительной работе, а не что основная функция проекта приостановлена.
...
![Status](https://img.shields.io/badge/status-5.4.16%20Testing-orange)
