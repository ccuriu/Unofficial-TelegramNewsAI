...
> [!NOTE]
> **Текущая версия — 5.4.13 Testing.** Offline-проверки и защитные regression tests пройдены; реальные испытания Telegram API продолжаются параллельно. Канал Testing означает, что сетевой профиль ещё проверяется в длительной работе, а не что основная функция проекта приостановлена.
...
Смысловой ML/embedding-поиск по Telegram-контенту в 5.4.13 Testing не используется. Для текущего масштаба локальный SQLite FTS5/LIKE даёт нужную скорость и простоту без тяжёлой модели; ИИ применяется на следующем этапе — для анализа уже подготовленного JSON.
